...
function removeProperties(obj, keys) {
  const mutated = { ...obj };
  keys.forEach((key) => delete mutated[key]);
  return mutated;
}
...
function makeWord(lettersObject) {
  ...
  const allLetters = [];
  entries.forEach(([letter, indexes]) => {
    indexes.forEach((index) => allLetters.push([index, letter]));
  });
  return allLetters
    .sort((a, b) => a[0] - b[0])
    .map((item) => item[1])
...
function sellTickets(queue) {
  const price = 25;
  let result = true;
  queue.reduce((revenue, banknote) => {
    if (banknote - price > revenue) result = false;
...
function group(array, keySelector, valueSelector) {
  ...
  values.forEach((value, index) => {
    result.get(keys[index]).push(value);
  });
  return result;
}
